...
- Goal IDs are derived from the title in `src/goal-selection.ts`, frozen at creation, and every ID a goal has ever had stays resolvable and reserved; any new field that stores a goal ID must resolve through `findGoalByStoredId` and be rewritten by `migrateProjectGoalIds`.
