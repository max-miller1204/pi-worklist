...
- Dependency edges are stored in one direction only, as `dependsOn` on the goal that waits; blocked, dependents, and cycles are all derived in `src/dependencies.ts` and must never become stored state.
- Nothing reachable from `src/cli.ts`, including the terminal board in `src/tui/`, may import `@earendil-works/*`; the compiled bin has to run with only Node and its declared runtime dependencies, never Pi peers.
